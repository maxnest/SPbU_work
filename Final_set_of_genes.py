...
parser.add_argument('--exp_table', type=argparse.FileType('r'), required=True,
                    help="Table with expression levels of selected GENES "
                         "(for instance, genes with averaged between replicates scaledTPM > 1)")
...
def filters(protein_dict, exp_dict, map_dict, min_len, wanted_dict, out):
    #   FIRST FILTER: noticeable expression level
    noticeable_expression_list = []
    noticeable_expression_list.extend(exp_dict.keys())
    ...
    print("After first filter (noticeable expression level) survived: {first}\n"
...
def output_writing(out, nucl_dict, prot_dict, header, exp_dict, wanted_dict):
    wanted_genes, wanted_transcripts, wanted_proteins = [], [], []
    with open("{out}.all_wanted_sequences.tsv".format(out=out), 'a') as all_wanted_output:
    ...
    with open("{out}.genes_level.after_filters.averaged_scaledTPM.tsv".format(out=out), 'a') as output_exp:
        output_exp.write("{header}\n".format(header="\t".join(header)))
        for gene, values in exp_dict.items():
            if gene in wanted_genes:
                ordered_values = [values[sample] for sample in header[1:]]
                output_exp.write("{gene}\t{ordered_values}\n".format(gene=gene, ordered_values="\t".join(
                    ordered_values)))


if __name__ == "__main__":
    nucl_dict, prot_dict = {}, {}
    header, exp_dict, map_dict, wanted_dict = [], {}, {}, {}
    print("##### Input files parsing #####")
    fasta_parsing(args.nucl_fasta, nucl_dict, "nucl")
    fasta_parsing(args.prot_fasta, prot_dict, "prot")
    expression_table_parsing(args.exp_table, header, exp_dict)
    map_parsing(map_dict, args.gene_trans_map)
    print("##### Applying filters #####")
    filters(prot_dict, exp_dict, map_dict, args.min_len, wanted_dict, args.out)
    print("##### Output files creation #####")
    output_writing(args.out, nucl_dict, prot_dict, header, exp_dict, wanted_dict)
