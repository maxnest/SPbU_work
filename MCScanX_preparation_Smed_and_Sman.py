...
def orthogroups_separation(orthogroups, orthodict, sp1_dict, sp2_dict, sp1_tag, sp2_tag):
    for line in orthogroups:
        ...
        for gene in genes.split(" "):
            ...
            elif gene.split(".p")[0] in sp1_dict.keys():
                orthodict[group_ID]["{sp1}".format(sp1=sp1_tag)].append(gene.split(".p")[0])
            elif gene.split(".")[0] in sp1_dict.keys():
                orthodict[group_ID]["{sp1}".format(sp1=sp1_tag)].append(gene.split(".")[0])
            elif gene in sp2_dict.keys():
                orthodict[group_ID]["{sp2}".format(sp2=sp2_tag)].append(gene)
            elif gene.split(".p")[0] in sp2_dict.keys():
                orthodict[group_ID]["{sp2}".format(sp2=sp2_tag)].append(gene.split(".p")[0])
