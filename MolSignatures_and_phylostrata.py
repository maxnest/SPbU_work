...
def merged_table_parsing(merged_table, merged_dict, phylostratr_dict):
    ...
    for line in merged_table:
        ...
        if geneID in phylostratr_dict.keys():
            merged_dict[sample].append(geneID)


def phylostratigraphy_content(phylostratr_dict, merged_dict, content_dict):
    phylostrata = set([value for value in phylostratr_dict.values()])
    samples = set([key for key in merged_dict.keys()])

    for phylostratum in phylostrata:
        content_dict[phylostratum] = {sample: [] for sample in samples}
...
if __name__ == "__main__":
    ...
    merged_table_parsing(args.merged_repr, merged_dict, phylostratr_dict)
