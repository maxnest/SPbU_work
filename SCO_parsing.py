...
def write_fasta(orthodict, fasta_seqs):
    ...
    for key, value in ortodict.items():
        with open("{key}.fasta".format(key=key), 'a') as output:
            output.write(">Csin_{key}\n{seq}\n".format(key=key, seq=fasta_dict[value[0]]))
            output.write(">Fhep_{key}\n{seq}\n".format(key=key, seq=fasta_dict[value[1]]))
            output.write(">Mlig_{key}\n{seq}\n".format(key=key, seq=fasta_dict[value[2]]))
            output.write(">Oviv_{key}\n{seq}\n".format(key=key, seq=fasta_dict[value[3]]))
            output.write(">Shae_{key}\n{seq}\n".format(key=key, seq=fasta_dict[value[4]]))
            output.write(">Sjap_{key}\n{seq}\n".format(key=key, seq=fasta_dict[value[5]]))
            output.write(">Sman_{key}\n{seq}\n".format(key=key, seq=fasta_dict[value[6]]))
            output.write(">Smed_{key}\n{seq}\n".format(key=key, seq=fasta_dict[value[7]]))
