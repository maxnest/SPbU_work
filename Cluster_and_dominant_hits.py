...
def cluster_analysis(ortho_dict, fd95_hits, x4_hits, x5_hits, cluster_with_hits):
        for cluster, values in ortho_dict.items():
            ...
            if len(all_hits) == 0:
                ...
                dominant_hit = []
                max_count = 0
                for el in set(all_hits):
                    if all_hits.count(el) == max_count:
                        dominant_hit.append(el)

                    elif all_hits.count(el) > max_count:
                        dominant_hit.clear()
                        dominant_hit.append(el)
                        max_count = dominant_hit.count(el)

                cluster_with_hits[cluster] = ",".join(set(dominant_hit))
