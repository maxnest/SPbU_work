...
def summary_table_parsing(summary_table, kegg_dict):
    ...
    for line in summary_table:
        description = line.strip().split("\t")
        gene, ec_number,  kegg_pathways, redia_exp, cercaria_exp, marita_exp = description[0], description[13],  \
            description[15], float(description[26]), float(description[27]), float(description[28])
            ...
            if kegg_pathway.startswith("ko"):
                for category, codes in kegg_dict.items():
                    if kegg_pathway.split("ko")[1] in codes:
                        if ec_number != "-":
                            codes[kegg_pathway.split("ko")[1]]["Total_count"].append(ec_number)

                            if redia_exp >= 1:
                                codes[kegg_pathway.split("ko")[1]]["Redia"].append(ec_number)

                            if cercaria_exp >= 1:
                                codes[kegg_pathway.split("ko")[1]]["Cercaria"].append(ec_number)

                            if marita_exp >= 1:
                                codes[kegg_pathway.split("ko")[1]]["Marita"].append(ec_number)


def output_writing(output, kegg_dict):
    with open("{output}.all_pathways_with_5_or_more_enzymes.tsv".format(output=output), 'a') as all_active_pathways:
        all_active_pathways.write("Category\tPathway\tRedia:EnzymesCount\tCercaria:EnzymesCount\tMarita:EnzymesCount\t"
                                  "Total_EnzymesCount\tRedia:Percent\tCercaria:Percent\tMarita:Percent\n")
        for category, codes in kegg_dict.items():
                ...
                if len(values["Total_count"]) >= 5:
                    all_active_pathways.write("{category}\t{pathway}\t{redia_count}\t{cercaria_count}\t{marita_count}\t"
                                              "{total}\t{redia_percent}\t{cercaria_percent}\t{marita_percent}\n".format(
                                                ...
                                                redia_count=len(set(values["Redia"])),
                                                cercaria_count=len(set(values["Cercaria"])),
                                                marita_count=len(set(values["Marita"])),
                                                total=len(set(values["Total_count"])),
                                                redia_percent=
                                                round((len(set(values["Redia"]))/len(set(values["Total_count"]))) * 100,
                                                      2),
                                                cercaria_percent=
                                                round((len(set(values["Cercaria"]))/len(set(values["Total_count"]))) * 100,
                                                      2),
                                                marita_percent=
                                                round((len(set(values["Marita"]))/len(set(values["Total_count"]))) * 100,
                                                      2)))
